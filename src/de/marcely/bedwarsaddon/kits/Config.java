...
public class Config {
    ...
    public static void load() {
        ...
        if (BwAdd.kitConfig.getRootTree().getChilds().size() == 0) {
            ...
            kit.addItem(new ItemStack(Material.
                    WOODEN_HOE));
            BedwarsAddonKits.kits.add(kit);
            save();
            return;
        ...
        for (final Tree t : BwAdd.kitConfig.getRootTree().getTreeChilds()) {
            ...
            for (final de.marcely.configmanager2.objects.Config c : t.getChilds()) {
                if (c.getName().equalsIgnoreCase("seticon")) {
                    final ItemStack is = Util.getItemItemstackByName(c.getValue());
                    if (is == null) {
                        continue;
                    }
                    kit2.setIcon(Util.renameItemstack(is, kit2.getName()));
                }
                else {
    ...
    public static void save() {
        ...
        BwAdd.kitConfig.addConfig("gui-title", (Object)Language.chatColorToString(BedwarsAddonKits.kitsGUITitle));
        ...
        BwAdd.kitConfig.addConfig("permissions-insufficient-item-name", (Object)Language.chatColorToString(BedwarsAddonKits.permissionsMissingItemName));
        BwAdd.kitConfig.addEmptyLine();
        BwAdd.kitConfig.addEmptyLine();
        BwAdd.kitConfig.addComment("### Messages");
        BwAdd.kitConfig.addConfig("message-message-no-permissions", (Object)Language.chatColorToString(BedwarsAddonKits.message_noPermissions));
        BwAdd.kitConfig.addConfig("message-set-kit", (Object)Language.chatColorToString(BedwarsAddonKits.message_setKit));
        BwAdd.kitConfig.addEmptyLine();
        BwAdd.kitConfig.addEmptyLine();
        BwAdd.kitConfig.addComment("### Kits:");
        for (final Kit kit : BedwarsAddonKits.kits) {
            BwAdd.kitConfig.addConfig(String.valueOf(kit.getName()) + ".setIcon", (Object)AUtil.toString(kit.getIcon()));
            for (final ItemStack is : kit.getItems()) {
                BwAdd.kitConfig.addConfig(String.valueOf(kit.getName()) + ".addItem", (Object)AUtil.toString(is));
            }
            BwAdd.kitConfig.addEmptyLine();
        }
